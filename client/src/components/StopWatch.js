...
class StopWatch extends React.Component {
  ...
  startButton = () => {
    ...
    axios.post("/api", createNewDate).then(res => {
  ...
  stopButton = () => {
    const createNewDate = new moment();
    axios
      .put("/api", { end_time: createNewDate, billable: "", unbillable: "" })
      .then(res => {});
  ...
  calculateTimeTotal = () => {
    const newTimeTotal = moment
      .utc(
        moment
          .duration(this.state.endTime.diff(this.state.startTime))
          .asMilliseconds()
      )
      .format("HH:mm:ss");
  ...
  createTimeEntry = () => {
    ...
    this.setState({
      timeEntry: {
        startEntry: start,
        endEntry: end,
        timeEntry: this.state.timeTotal
      }
    });
  ...
  render() {
    ...
    return (
      <>
        <Button color="green" size="mini" onClick={() => this.startButton()}>
          Start
        </Button>
        <Button color="red" size="mini" onClick={() => this.stopButton()}>
          Stop
        </Button>
      </>
    );
